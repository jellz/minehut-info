...
class ServerInfo extends Component {
	...
	async componentDidMount() {
		...
		let res = await fetch(
			'https://api.minehut.com/server/' + this.state.serverName + '?byName=true'
		);
	...
	render() {
		return (
			<div>
				<Title
					style={{ fontSize: 50, fontFamily: 'Righteous' }}
				...
				{this.state.error !== null && (
					<Title className='has-text-danger'>{this.state.error}</Title>
				)}
